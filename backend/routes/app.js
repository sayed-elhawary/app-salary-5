...
router.post('/upload', authMiddleware, upload.single('file'), async (req, res) => {
  try {
    ...
    // إنشاء سجلات لكل موظف وكل يوم في النطاق الزمني
      ...
      while (currentDate <= endDate) {
        ...
        if (existingReport) {
          ...
          try {
            ...
            console.error(`Error processing report for code ${existingReport.code}:`, err.message);
            skippedCount++;
          }
        } else {
          // يوم مفقود
          ...
          const reportData = {
            ...
            lateMinutes: 0,
            lateDeduction: 0,
            earlyLeaveDeduction: isWeekly ? 0 : 1, // خصم إذا لم يكن إجازة أسبوعية
            absence: isWeekly ? false : true, // غياب إذا لم يكن إجازة أسبوعية
            annualLeave: false,
            medicalLeave: false,
            officialLeave: false,
            leaveCompensation: 0,
          ...
          if (!existingReportInDB) {
            ...
            await fingerprint.save();
            createdCount++;
            console.log(`Created fingerprint for code ${fingerprint.code} on ${dateStr}`);
          }
        }
...
router.get('/', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (dateFrom && dateTo) {
      ...
      users = code ? await User.find({ code }) : await User.find();
...
router.post('/appropriate-value', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    if (!dateFrom || !dateTo || !appropriateValue) {
      console.error('Missing required fields:', { code, dateFrom, dateTo, appropriateValue });
      return res.status(400).json({ error: 'جميع الحقول مطلوبة: الكود (اختياري)، تاريخ البداية، تاريخ النهاية، القيمة المناسبة' });
    }

    // تحويل التواريخ إلى تنسيق Luxon مع ضبط المنطقة الزمنية
    const startDate = DateTime.fromISO(dateFrom, { zone: 'Africa/Cairo' });
    const endDate = DateTime.fromISO(dateTo, { zone: 'Africa/Cairo' });
    ...
    // جلب المستخدمين (إما مستخدم واحد بكود معين أو كل المستخدمين)
    const users = code ? [await User.findOne({ code })] : await User.find();
    if (code && !users[0]) {
      console.error(`User not found for code ${code}`);
      return res.status(404).json({ error: 'المستخدم غير موجود' });
    ...
    for (const user of users) {
      ...
      while (currentDate <= endDate) {
        ...
        // التأكد من أن اليوم ليس إجازة أسبوعية أو يوم 8 مايو 2025
        if (!isWeekly) {
          const existingReport = await Fingerprint.findOne({
            code: user.code,
            date: {
...
router.post('/official-leave', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    // تحويل التواريخ إلى تنسيق Luxon
    const startDate = DateTime.fromISO(dateFrom, { zone: 'Africa/Cairo' });
    const endDate = DateTime.fromISO(dateTo, { zone: 'Africa/Cairo' });
...
// مسار إضافة بدل إجازة
router.post('/leave-compensation', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { code, dateFrom, dateTo } = req.body;
    ...
    const reports = [];
    for (const user of users) {
      // حساب قيمة بدل الإجازة: (الراتب الأساسي / 30) * 2
      const leaveCompensationValue = (user.basicSalary || 3000) / 30 * 2;
...
// مسار إضافة إجازة طبية
router.post('/medical-leave', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { code, dateFrom, dateTo } = req.body;
      ...
      while (currentDate <= endDate) {
        ...
        if (!isWeekly) {
          ...
          if (existingReport) {
            ...
            fingerprint.officialLeave = false;
            fingerprint.leaveCompensation = 0;
            fingerprint.medicalLeaveDeduction = user.medicalLeaveDeduction || 50; // افتراضي أو من إعدادات المستخدم
            fingerprint.appropriateValue = 0;
            fingerprint.appropriateValueDays = 0;
            fingerprint.employeeName = user.fullName || 'غير معروف';
            fingerprint.monthlyLateAllowance = user.monthlyLateAllowance || 120;
            ...
            fingerprint = new Fingerprint({
              ...
              officialLeave: false,
              leaveCompensation: 0,
              medicalLeaveDeduction: user.medicalLeaveDeduction || 50,
              appropriateValue: 0,
              appropriateValueDays: 0,
              isSingleFingerprint: false,
              workDaysPerWeek,
...
// مسار إضافة القيمة المناسبة (موجود من قبل)
router.post('/appropriate-value', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { code, dateFrom, dateTo, appropriateValue } = req.body;
...
router.post('/annual-leave', authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { code, dateFrom, dateTo } = req.body;
    ...
    // حساب عدد الأيام غير الإجازات الأسبوعية
    let leaveDays = 0;
    let currentDate = startDate;
    while (currentDate <= endDate) {
      const workDaysPerWeek = 6; // افتراضي، سيتم تحديثه لكل مستخدم لاحقًا
      if (!isWeeklyLeaveDay(currentDate.toJSDate(), workDaysPerWeek)) {
        leaveDays++;
      }
      currentDate = currentDate.plus({ days: 1 });
    ...
    for (const user of users) {
      const balance = user.annualLeaveBalance || 0;
      if (balance < leaveDays) {
        console.error(`Insufficient annual leave balance for user ${user.code}: ${balance} days available, ${leaveDays} days requested`);
        return res.status(400).json({ error: `رصيد الإجازة السنوية غير كافٍ للمستخدم ${user.fullName} (${balance} أيام متبقية)` });
      }
    }
    ...
    for (const user of users) {
      ...
      // خصم الأيام من رصيد الإجازة السنوية
      if (daysDeducted > 0) {
        user.annualLeaveBalance = (user.annualLeaveBalance || 0) - daysDeducted;
        await user.save();
        console.log(`Deducted ${daysDeducted} days from annual leave balance for user ${user.code}. New balance: ${user.annualLeaveBalance}`);
      }
    }

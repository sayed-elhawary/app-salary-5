...
// Middleware للتحقق من صلاحية المستخدم (يجب أن يكون أدمن)
...
// دالة للتحقق إذا كان اليوم إجازة أسبوعية
...
// دالة لحساب أيام الإجازة الأسبوعية في فترة معينة
...
async function handleLateDeduction(report) {
  try {
    ...
    if (report.annualLeave || report.medicalLeave || isWeeklyLeaveDay(report.date, user ? user.workDaysPerWeek : 6)) {
...
// دالة لحساب خصم الانصراف المبكر
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (checkIn && !timeRegex.test(checkIn)) {
      console.error('Invalid checkIn format:', checkIn);
      return res.status(400).json({ error: 'تنسيق وقت الحضور غير صالح، يجب أن يكون HH:mm:ss' });
    }
    if (checkOut && !timeRegex.test(checkOut)) {
      console.error('Invalid checkOut format:', checkOut);
      return res.status(400).json({ error: 'تنسيق وقت الانصراف غير صالح، يجب أن يكون HH:mm:ss' });
    }

    // التحقق من وجود سجل بنفس الكود والتاريخ
    ...
    if (existingReport) {
      ...
      fingerprint.checkIn = checkIn ? DateTime.fromFormat(checkIn, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate() : null;
      fingerprint.checkOut = checkOut ? DateTime.fromFormat(checkOut, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate() : null;
      ...
      fingerprint = new Fingerprint({
        ...
        checkIn: checkIn ? DateTime.fromFormat(checkIn, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate() : null,
        checkOut: checkOut ? DateTime.fromFormat(checkOut, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate() : null,
    ...
    } else if (!annualLeave && existingReport && existingReport.annualLeave) {
    ...
    console.error('Error creating/updating fingerprint:', error.message, error.stack);
    res.status(500).json({ error: 'خطأ في إنشاء/تحديث السجل', details: error.message });
  }
});

// مسار لتحميل ملف البصمات
    ...
    for (const report of reports) {
      try {
        ...
        if (existingReport) {
          ...
          fingerprint.workDaysPerWeek = user ? user.workDaysPerWeek : 6;
          fingerprint.annualLeave = false;
          fingerprint.medicalLeave = false;
          fingerprint.medicalLeaveDeduction = 0;
          await fingerprint.calculateAttendance();
        ...
        while (currentDate <= endDate) {
          ...
          if (!existingReport) {
            if (isWeeklyLeaveDay(currentDate.toJSDate(), defaultWorkDaysPerWeek)) {
              ...
              if (!existingWeeklyReport) {
                console.log(`Creating weekly leave report for ${code} on ${dateStr}`);
                const fingerprint = new Fingerprint(weeklyLeaveReport);
                await fingerprint.calculateAttendance();
                await fingerprint.save();
                missingReports.push(fingerprint);
              ...
              if (!existingAbsenceReport) {
                ...
                console.log(`Creating absence report for ${code} on ${dateStr}`);
                const fingerprint = new Fingerprint(absenceReport);
                await fingerprint.calculateAttendance();
                await fingerprint.save();
                missingReports.push(fingerprint);
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (
        ...
        totals.totalWeeklyLeaveDays !==
        totalDays
      ) {
        totals.totalWeeklyLeaveDays =
          totalDays -
          (totals.totalWorkDays + totals.totalAbsenceDays + totals.totalAnnualLeaveDays + totals.totalMedicalLeaveDays);
      }
      ...
      const deductionsValue = (
        (totals.totalAbsenceDays + totals.lateDeductionDays + totals.earlyLeaveDeductionDays + totals.medicalLeaveDeductionDays) * dailySalary +
        user.penaltiesValue +
        user.violationsInstallment
      ).toFixed(2);
  ...
  try {
    ...
    if (existingReport) {
      ...
      targetFingerprint.checkIn = checkIn
        ? DateTime.fromFormat(checkIn, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate()
        : existingReport.checkIn;
      targetFingerprint.checkOut = checkOut
        ? DateTime.fromFormat(checkOut, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate()
    ...
    if (targetFingerprint.annualLeave) {
      ...
      targetFingerprint.checkIn = DateTime.fromObject({ hour: 8, minute: 30 }, { zone: 'Africa/Cairo' }).toJSDate();
      targetFingerprint.checkOut = DateTime.fromObject({ hour: 17, minute: 30 }, { zone: 'Africa/Cairo' }).toJSDate();
...
router.post('/mission', authMiddleware, async (req, res) => {
  try {
    ...
    let fingerprint;
    if (existingReport) {
      console.log(`Updating existing mission report for code ${code} on ${dateDt.toISODate()}`);
      fingerprint = existingReport;
      fingerprint.checkIn = checkIn
        ? DateTime.fromFormat(checkIn, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate()
        : existingReport.checkIn;
      fingerprint.checkOut = checkOut
        ? DateTime.fromFormat(checkOut, 'HH:mm:ss', { zone: 'Africa/Cairo' }).toJSDate()
        : existingReport.checkOut;
      fingerprint.absence = false;
      fingerprint.annualLeave = false;
    ...
    res.json({
      message: existingReport ? 'تم تحديث المأمورية بنجاح' : 'تم إنشاء المأمورية بنجاح',
      report: responseReport,
    });
  } catch (error) {

...
const EditModal = ({ report, isOpen, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAnnualLeaveBalance(response.data.annualLeaveBalance);
      setShowSuccess(true);
      setTimeout(() => {
    ...
    >
      <AnimatePresence>
        {showSuccess ? (
          ...
          <motion.div
            className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300 w-full max-w-lg"
          ...
          >
            ...
            {error && (
              <motion.p
                ...
                exit={{ opacity: 0, y: -10 }}
                className="bg-red-50 text-red-600 p-2 rounded-md text-right mb-4 text-sm font-amiri"
              >
                {error}
              </motion.p>
            )}
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label className="block text-gray-700 text-sm font-medium mb-1 text-right font-amiri">
                    كود الموظف
                  </label>
                  <input
                    type="text"
                    value={report.code}
                    readOnly
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100 text-right text-sm font-amiri focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-gray-700 text-sm font-medium mb-1 text-right font-amiri">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-right text-sm font-amiri focus:ring-2 focus:ring-teal-500 focus:border-teal-500 hover:bg-gray-100 transition-colors duration-200 disabled:bg-gray-100 disabled:cursor-not-allowed"
                    step="1"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">الغياب</label>
                  <input
                    type="checkbox"
                    checked={absence}
                ...
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">إجازة سنوية</label>
                  <input
                    type="checkbox"
                    checked={annualLeave}
                ...
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">إجازة طبية</label>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">إجازة رسمية</label>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">بدل الإجازة</label>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-end">
                  <label className="text-gray-700 text-sm font-medium mr-2 font-amiri">قيمة مناسبة</label>
                  <input
                    type="checkbox"
              ...
              <div className="flex flex-wrap justify-end gap-2 sm:gap-3">
                <motion.button
                  ...
                  className={`w-full sm:w-auto bg-teal-500 text-white px-5 py-2.5 rounded-md hover:bg-teal-600 transition-all duration-200 text-sm font-amiri shadow-sm ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                ...
                <motion.button
                  ...
                  whileHover={{ scale: loading ? 1 : 1.02 }}
                  whileTap={{ scale: loading ? 1 : 0.98 }}
                  className={`w-full sm:w-auto bg-gray-300 text-gray-700 px-5 py-2.5 rounded-md hover:bg-gray-400 transition-all duration-200 text-sm font-amiri shadow-sm ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  إلغاء
                </motion.button>
              </div>
